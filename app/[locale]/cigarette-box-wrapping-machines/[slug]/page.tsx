...
const CAT_PATH = "/cigarette-box-wrapping-machines";
...
export function generateMetadata({
  ...
  const base = getProduct(CATEGORY, params.slug);
  if (!base) return {};
...
export default function LocaleWrappingProductPage({
  params,
}: {
  params: { locale: LocaleCode; slug: string };
}) {
  ...
  const related = getProductsByCategory(CATEGORY).filter((p) => p.slug !== base.slug);

  return (
    <ProductDetail
      product={product}
      locale={locale}
      categoryLabel={dict.categories[CAT_PATH].eyebrow}
      crumbs={[
